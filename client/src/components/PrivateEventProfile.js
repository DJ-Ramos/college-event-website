...
import { CreatePrivateEventAPI } from "../api_handler/Private";
import { Button, Modal, Nav, Form } from "react-bootstrap";

const PrivateEventProfile = (university_id) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const data = await CreatePrivateEventAPI(formValues);
  ...
  return (
    <>
      <Nav.Link onClick={handleShow}>
        <p className="h5">Create Private Event</p>
      </Nav.Link>

      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Create an Private Event</Modal.Title>
        </Modal.Header>
